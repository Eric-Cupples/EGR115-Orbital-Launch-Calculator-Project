function [AltitudeGraph, ThetaGraph, CargoAltitudeGraph, CargoThetaGraph, MaxAlt] = GraphFunction_2Stage(GraphArray, CargoGraphArray, GraphHold, GoalPerigee, GoalAngle) %<SM:PDF_PARAM>  %<SM:PDF_RETURN> 
...
%% Rocket Graphs
    ...
    AltitudeGraph = plot(DeltaTime, AltitudeGraphY); %<SM:PLOT> 
        hold on;
            ...
            Stall = plot(DeltaTime(index), AltitudeGraphY(index), ':c');
            index = (DeltaTime >= GraphHold(1,2)+GraphHold(1,1));
            Stage2 = plot(DeltaTime(index), AltitudeGraphY(index), 'c');
...
%% Cargo Graphs
    ...
    CargoAltitudeGraph = plot(CargoDeltaTime, CargoAltitudeGraphY);
        hold on;
            index = (DeltaTime >= GraphHold(1,1));
            Stall = plot(DeltaTime(index), CargoAltitudeGraphY(index), ':c');
            index = (DeltaTime >= GraphHold(1,2)+GraphHold(1,1));
            Stage2 = plot(DeltaTime(index), CargoAltitudeGraphY(index), 'c');
