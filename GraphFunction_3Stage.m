function [AltitudeGraph, ThetaGraph, CargoAltitudeGraph, CargoThetaGraph, MaxAlt] = GraphFunction_3Stage(GraphArray, CargoGraphArray, GraphHold, GoalPerigee, GoalAngle) %<SM:PDF_PARAM>  %<SM:PDF_RETURN>
...
%Altitude Graph
subplot(2,3,1);
AltitudeGraph = plot(DeltaTime, AltitudeGraphY); %<SM:PLOT>
hold on;
    index = (DeltaTime >= GraphHold(1,1));
    Stall2 = plot(DeltaTime(index), AltitudeGraphY(index), ':c');
    index = (DeltaTime >= GraphHold(1,2)+GraphHold(1,1));
    Stage3 = plot(DeltaTime(index), AltitudeGraphY(index), 'c');
    
    index = (DeltaTime >= GraphHold(1,3)+GraphHold(1,2)+GraphHold(1,1));
    Stall = plot(DeltaTime(index), AltitudeGraphY(index), ':b');
    index = (DeltaTime >= GraphHold(1,4) + GraphHold(1,3)+GraphHold(1,2)+GraphHold(1,1));
    Stage2 = plot(DeltaTime(index), AltitudeGraphY(index), 'b');
    ...
    if GoalPerigee ~= 607611.6588
        legend({'Stage 1', 'Stall', 'Stage 2', 'Stall2', 'Stage 3', 'Max Height', 'LEO', 'Goal'},'Location','northwest','NumColumns',2);
    else
        legend({'Stage 1', 'Stall', 'Stage 2', 'Stall2', 'Stage 3', 'Max Height', 'LEO'},'Location','northwest','NumColumns',2);
    end
hold off;
...
hold on;
    ...
    yBox = [75, 90, 90, 75, 75];
    patch(xBox, yBox, 'red', 'FaceColor', 'red', 'FaceAlpha', 0.1); %<SM:NEWFUN>
...
hold on;
    index = (DeltaTime >= GraphHold(1,1));
    Stall2 = plot(DeltaTime(index), VelocityGraphY(index), ':c');
    index = (DeltaTime >= GraphHold(1,2)+GraphHold(1,1));
    Stage3 = plot(DeltaTime(index), VelocityGraphY(index), 'c');
    
    index = (DeltaTime >= GraphHold(1,3)+GraphHold(1,2)+GraphHold(1,1));
    Stall = plot(DeltaTime(index), VelocityGraphY(index), ':b');
    index = (DeltaTime >= GraphHold(1,4) + GraphHold(1,3)+GraphHold(1,2)+GraphHold(1,1));
    Stage2 = plot(DeltaTime(index), VelocityGraphY(index), 'b');
...
hold on;
    index = (DeltaTime >= GraphHold(1,2));
    Stall2 = plot(DeltaTime(index), CargoAltitudeGraphY(index), ':c');
    index = (DeltaTime >= GraphHold(1,1));
    Stage3 = plot(DeltaTime(index), CargoAltitudeGraphY(index), 'c');
    
    index = (DeltaTime >= GraphHold(1,3)+GraphHold(1,4)+GraphHold(1,5));
    Stall = plot(DeltaTime(index), CargoAltitudeGraphY(index), ':b');
    index = (DeltaTime >= GraphHold(1,2) + GraphHold(1,3)+GraphHold(1,4)+GraphHold(1,5));
    Stage2 = plot(DeltaTime(index), CargoAltitudeGraphY(index), 'b');
    ...
    xlim([0, length(DeltaTime)]);
    ylim([0,3000000]);
    if max(CargoAltitudeGraphY) > 3000000
        ylim([0, max(CargoAltitudeGraphY+5000)]);
    end
...
hold on;
    index = (DeltaTime >= GraphHold(1,1));
    Stall2 = plot(DeltaTime(index), CargoVelocityGraphY(index), ':c');
    index = (DeltaTime >= GraphHold(1,2)+GraphHold(1,1));
    Stage3 = plot(DeltaTime(index), CargoVelocityGraphY(index), 'c');
    
    index = (DeltaTime >= GraphHold(1,3)+GraphHold(1,2)+GraphHold(1,1));
    Stall = plot(DeltaTime(index), CargoVelocityGraphY(index), ':b');
    index = (DeltaTime >= GraphHold(1,4) + GraphHold(1,3)+GraphHold(1,2)+GraphHold(1,1));
    Stage2 = plot(DeltaTime(index), CargoVelocityGraphY(index), 'b');
